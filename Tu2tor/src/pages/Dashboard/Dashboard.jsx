...
const Dashboard = () => {
  const { user } = useAuth();
  const { bookings, reviews, fetchBookings, fetchTutors } = useApp();
  ...
  }, [user?.userId]); // Only re-fetch when user changes

  // Get user's bookings
  // API returns: booking.student.id and booking.tutor.userId
  const userBookings = bookings.filter(
    b => b.student?.id === user?.id || b.tutor?.userId === user?.id
  );
